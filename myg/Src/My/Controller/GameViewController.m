...
#define Margin 20
#define FontSize 14
@interface GameViewController ()<UITableViewDelegate,UITableViewDataSource,UserAddressCellDelegate>
{
...
- (void)pushAddAddressViewController{
    ...
    AddGameController * addAddress = [[AddGameController alloc]init];
    ...
    if ([data isKindOfClass:[NSDictionary class]])
    {
        if([data[@"code"] isEqualToString:@"200"])
        {
            ...
            [array enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                ...
                DebugLog(@"**+**%@",_model.idd);
                if (!IsStrEmpty(_model.sheng) || !IsStrEmpty(_model.qq)) {
                    _model = nil;
                }else{
